...
def vid_predict(clock, scale):
    # 🧬 FIXED: Lowered floor constraint boundary to 2000 to process underclocks cleanly
    if clock < 2000:
        raise ValueError("cannot predict vid for clocks below 2 GHz")
    p = -1.519 + scale * 0.004325
    q = 2800.0 - (scale * 10.0)
    return 0.0003 * clock * clock + p * clock + q

def vid_predict_delta(clock_cur, clock_next, scale_cur, scale_next):
    # 🧬 FIXED: Lowered predictive delta bounds to prevent logic bypass loops
    if clock_cur < 2000 or clock_next < 2000:
        return 0
...
def detect_active_cores():
    ...
    for i in range(0, 8):
        ...
        # 🧬 FIXED: Lowered core telemetry evaluation ceiling from 3000 down to 2000
        # This stops the script from reporting "XXXXXXXX" (faking active core states)
        if clk > 2000:
            print(i, end='')
            cores.append(True)
        else:
...
def detect(f_target, v_max, t_max):
    ...
    # 🧬 FIXED: Stepped up long stress timeout loop directly to your true 150-second production standard
    delay_long = 150
    ...
    while True:
        ...
        if(check_throttling(f_test - f_throttling_threshold, cores)):
            stress_stop()
            print("Aborting because throttling was detected")
            break
